import type { ProductAnalysisOutput } from '../schemas/product-analysis.ts'

export type PromptLanguage = 'zh-CN' | 'en' | 'ja' | 'ko'
export type PlanPlatform = 'ecommerce' | 'xiaohongshu'
...
const platformLabels: Record<PlanPlatform, string> = {
  ecommerce: '通用电商',
  xiaohongshu: '小红书',
}
...
export function buildVisualStyleGuidePrompt(
  analysis: ProductAnalysisOutput,
  options: { style?: string; platform?: PlanPlatform; language?: PromptLanguage } = {},
): string {
  const style = options.style ?? 'premium'
  const platform = options.platform ?? 'ecommerce'
  const language = options.language ?? 'zh-CN'
