...
import { inferCategory, noopLogger, type Logger, type UsageEvent } from "../../ports/logger.ts";
...
export class OpenAICompatibleAdapter implements ProviderAdapter {
  ...
  // Explicit fields rather than constructor parameter properties, because the
  // project sets `erasableSyntaxOnly` (parameter properties are not erasable).
  private readonly baseUrl: string;
  private readonly apiKey: string;
  private readonly logger: Logger;

  constructor(baseUrl: string, apiKey: string, logger: Logger = noopLogger) {
  ...
  private async logUsage(params: {
    ...
    try {
      ...
      // Usage accounting must never break a generation call.
    }
  }
  ...
  private async requestRaw(
    ...
    // NOTE (fixed during the DSH port): upstream read
    //   `if (urls.length === 1 || options?.suppressUsageLog)`
    // which conflated "skip usage logging" with "skip the base-URL retry".
    // The Visual Prompt Agent is the only caller that passes
    // `suppressUsageLog: true` (visual-prompt-agent.ts), so it silently lost
    // the `/v1`-vs-root fallback and degraded to the template prompt whenever
    // a gateway needed the versioned base URL. Retry and logging are now
    // independent: the loop always walks the candidates, and only the
    // consolidated log entry below honours the suppression flag.
    if (urls.length === 1) {
