...
function createLlmCompleteJson(llm) {
	const rec = llm;
	return async (input) => {
		...
		if (!model && typeof rec.listModels === "function") try {
			...
			if (Array.isArray(models) && models.length > 0) {
				...
				const picked = (input.images?.length ? withImage : void 0) ?? models[0];
		...
		for (const image of input.images ?? []) userContent.push({
			type: "image_url",
...
function resolveCompleteJson(opts) {
	...
	if (llmLooksUsable(opts.llm)) {
		const llmCaller = createLlmCompleteJson(opts.llm);
...
const HERO_FALLBACK = [
	{
		...
		visualPrompt: "Primary Prompt: 1:1 电商首张主视觉，商品以 3/4 角度居中偏下。几何不可反转，禁止逆风，主体与参考图一致，避免乱码文字。\nEnglish Prompt: Square e-commerce primary hero. Preserve real product geometry.",
		editableFields: {
			tone: "高级质感",
	...
	},
	{
		...
		visualPrompt: "Primary Prompt: 1:1 核心卖点头图，近景产品 + 功能分解标注。几何不可反转，禁止逆风，主体与参考图一致。\nEnglish Prompt: Square selling-point hero with functional annotations.",
	...
	},
	{
		...
		visualPrompt: "Primary Prompt: 1:1 场景氛围头图，商品放入真实使用场景。主体与参考图一致，禁止逆风。\nEnglish Prompt: Square lifestyle hero in a real usage scene.",
	...
	},
	{
		...
		visualPrompt: "Primary Prompt: 电商头图，强调品质细节。避免乱码文字，主体与参考图一致。\nEnglish Prompt: Square hero focused on craftsmanship.",
...
function pad(items, fallback, count) {
...
function normalizeSections(plan, heroCount, detailCount) {
