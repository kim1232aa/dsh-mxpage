...
*/
function withChannelHints(body, model, aspectRatio) {
	if (!isOpenAiGptImageModel$1(model)) body.response_format = "b64_json";
	if (isGrokImageModel(model) && aspectRatio) body.aspect_ratio = aspectRatio;
	return body;
}
	...
	*/
	...
	async generateImage(input) {
		...
		if (referenceImages.length > 0) {
			...
			const hint = (body) => withChannelHints(body, input.model, input.aspectRatio);
...
function sniffImageMime(bytes) {
	if (bytes.length > 3 && bytes[0] === 137 && bytes[1] === 80) return "image/png";
	if (bytes.length > 3 && bytes[0] === 255 && bytes[1] === 216) return "image/jpeg";
	if (bytes.length > 12 && bytes[0] === 82 && bytes[8] === 87) return "image/webp";
...
function createAssetStore(host) {
	...
	return {
		...
		async saveGeneratedImage(params) {
			...
			if (!bytes) throw new Error("Image generation produced no usable image output.");
			const sniffed = params.source.svgText ? null : sniffImageMime(bytes);
			const mimeType = params.source.svgText ? "image/svg+xml" : sniffed ?? params.source.mimeType ?? "image/png";
...
function normalizeModelId$1(value) {
...
function pickAnalysisModel(provider, preferredModelId) {
	...
	return findMatch(textVisionModels, (item) => item.isDefaultAnalysis && isStableAnalysisCandidate(item.modelId)) ?? findMatch(textVisionModels, (item) => normalizeModelId$1(item.modelId).includes("gemini") && isStableAnalysisCandidate(item.modelId)) ?? findMatch(textVisionModels, (item) => normalizeModelId$1(item.modelId).includes("gpt-4o") && isStableAnalysisCandidate(item.modelId)) ?? findMatch(textVisionModels, (item) => isStableAnalysisCandidate(item.modelId)) ?? findMatch(textModels, (item) => item.isDefaultAnalysis) ?? findMatch(textModels, (item) => isStableAnalysisCandidate(item.modelId)) ?? textModels[0]?.modelId ?? null;
}
...
function hasRealImageGeneration$1(model) {
	return readCapabilities$1(model).real_image_gen !== false;
}
function canGenerateRealImage(model) {
	const modelId = model.modelId ?? "";
	return hasImageCapability(model) && (hasRealImageGeneration$1(model) || isGeminiImageModel(modelId) || isOpenAiGptImageModel(modelId));
}
...
//#endregion
//#region src/host/jobs-task-runner.ts
const MXPAGE_JOB_KIND = "mxpage_page";
function createJobsTaskRunner(options) {
		...
		status(id) {
			...
			try {
				...
				switch (snapshot.status) {
					...
					case "completed":
					case "failed":
					case "killed": return snapshot.status;
...
function createQueuedTaskRunner(options) {
	...
	*/
	...
	function recordSettled(id, state) {
		settled.delete(id);
...
//#endregion
//#region src/host/fallback-task-runner.ts
const NO_CONTROLLER = /no job controller|background jobs unavailable/i;
function createFallbackTaskRunner(primary, fallback) {
	...
	return {
		...
		get(id) {
			return used.get(id)?.get?.(id) ?? primary.get?.(id) ?? fallback.get?.(id);
		},
		status(id) {
			const known = used.get(id)?.status?.(id);
...
function createMxpageRuntime(config, options = {}) {
	...
	const runner = options.jobs ? createFallbackTaskRunner(createJobsTaskRunner({
		jobs: options.jobs,
		repository,
...
//#endregion
//#region src/core/ai/model-matcher.ts
function sortByScore(models, score) {
...
function findGptImage2Model(models) {
	return findPreferredModel(models, ["gpt-image-2", "gptimage2"]) ?? sortByScore(models.filter((model) => /^gpt[-_\s]?image[-_\s]?2(?:[-_\s]|$)/i.test(model.modelId)), (model) => model.modelId.toLowerCase() === "gpt-image-2" ? 100 : 90)[0] ?? null;
}
...
function findCostEffectiveTextModel(models) {
	return findPreferredModel(models, COST_EFFECTIVE_TEXT_MODELS);
}
function scoreTextCandidate(model) {
	...
	if (/(image|imagen|dall|flux|recraft|seedream|jimeng|midjourney|ideogram|cogview|audio|tts|speech|embedding|rerank|moderation)/i.test(id)) score -= 12;
	return score;
}
function scoreImageCandidate(model) {
	const id = model.modelId.toLowerCase();
	let score = 0;
	...
	if (model.capabilities.high_quality) score += 5;
	if (/(gpt[-_\s]?image|chatgpt-image|banana|nano-banana|imagen|dall[-_\s]?e|flux|recraft|seedream|jimeng|midjourney|ideogram|hidream|kolors|wanx|cogview)/i.test(id)) score += 10;
	if (/(pro|max|ultra|quality)/i.test(id)) score += 3;
	if (/(mini|flash|lite)/i.test(id)) score += 1;
	if (isPreviewOrTest(id)) score -= 6;
	return score;
}
function recommendDefaultModels(models) {
	const visionModels = models.filter(isVisionCandidate);
	const textModels = models.filter(isTextGenerationCandidate);
	const stableImageGenerationModels = models.filter((item) => hasRealImageGeneration(item) && isStableImageCandidate(item.modelId));
	const imageGenerationModels = models.filter((item) => hasRealImageGeneration(item));
	const stableImageEditModels = models.filter((item) => hasRealImageEdit(item) && isStableImageCandidate(item.modelId));
	const imageEditModels = models.filter((item) => hasRealImageEdit(item));
	const analysisModel = findCostEffectiveTextModel(visionModels) ?? sortByScore(visionModels.filter((item) => !isPreviewOrTest(item.modelId)), scoreTextCandidate)[0] ?? sortByScore(visionModels, scoreTextCandidate)[0] ?? null;
	const planningModel = findCostEffectiveTextModel(textModels) ?? sortByScore(textModels.filter((item) => item.capabilities.structured_output && !isPreviewOrTest(item.modelId)), scoreTextCandidate)[0] ?? sortByScore(textModels.filter((item) => !isPreviewOrTest(item.modelId)), scoreTextCandidate)[0] ?? sortByScore(textModels, scoreTextCandidate)[0] ?? null;
	const heroImageModel = findGptImage2Model(imageGenerationModels) ?? sortByScore(stableImageGenerationModels, scoreImageCandidate)[0] ?? sortByScore(imageGenerationModels, scoreImageCandidate)[0] ?? null;
	const detailImageModel = findGptImage2Model(imageGenerationModels) ?? sortByScore(stableImageGenerationModels.filter((item) => /detail|pro|max|ultra|quality|gpt[-_\s]?image|imagen|flux|dall[-_\s]?e|seedream|jimeng|midjourney|ideogram|cogview/i.test(item.modelId)), scoreImageCandidate)[0] ?? sortByScore(stableImageGenerationModels, scoreImageCandidate)[0] ?? heroImageModel;
	const imageEditModel = findGptImage2Model(imageEditModels) ?? sortByScore(stableImageEditModels, scoreImageCandidate)[0] ?? sortByScore(imageEditModels, scoreImageCandidate)[0] ?? null;
	return {
...
//#endregion
//#region src/core/monitor/api-usage.ts
/**
...
*/
function normalizeLedgerEntry(raw, fallbackId) {
	if (!raw || typeof raw !== "object") return null;
	const record = raw;
	const status = typeof record.status === "number" ? record.status : 0;
	const ok = record.ok === true;
	const errorMessage = typeof record.errorMessage === "string" && record.errorMessage.trim() ? record.errorMessage : null;
	const category = typeof record.category === "string" && record.category ? record.category : "unknown";
	return {
...
function classifyQuotaState(statusCode, body) {
	const text = body.toLowerCase();
	if (/monthly spending limit|spending limit|quota|insufficient_quota|billing|可用余额不足|余额不足|充值后再使用/i.test(body)) return "spending_limited";
	if (statusCode === 429 || /rate limit|限流/i.test(body)) return "rate_limited";
	if (statusCode === 401 || statusCode === 403 || /invalid token|unauthorized|forbidden/i.test(text)) return "auth_error";
	if (statusCode >= 400) return "other_error";
	return "ok";
}
...
function extractRawMessage(value) {
	...
	if (embeddedJsonMatch) {
		const embedded = tryParseJson(embeddedJsonMatch[0]);
...
function fallbackMessageByStatus(statusCode) {
	if (statusCode === 401 || statusCode === 403) return "当前 API Key 无效、无权限，或已被额度策略拒绝。";
...
function humanizeApiMonitorMessage(input) {
	...
	if (/monthly spending limit|spending limit|insufficient_quota|quota.*exceed|quota exceeded|billing|可用余额不足|余额不足|充值后再使用/i.test(raw)) return "当前 API Key 已达到月度额度上限，请前往代理商后台提高或取消月度限额，或更换可用 Key。";
	if (statusCode === 429 || /rate limit|触发限流|too many requests/i.test(raw)) return "当前请求触发了限流，请稍后重试或降低调用频率。";
	if (statusCode === 401 || /invalid token|unauthorized|forbidden|invalid api key|api key/i.test(normalized)) return "当前 API Key 无效或无权限，请检查 API Key、代理地址和账户权限。";
	if (/timed out|timeout|network error|fetch failed|socket hang up|econnreset|enotfound|econnrefused/i.test(normalized)) return "网络请求失败或响应超时，请检查代理地址是否可达，或稍后重试。";
	if (/this operation was aborted|aborterror/i.test(normalized)) return "请求已被中止，通常是超时控制或探测流程主动停止。";
	if (/no available endpoint found|get instance failed|real image generation endpoint/i.test(normalized)) return "当前代理商没有为该模型提供可用端点，请更换模型或 Provider。";
	if (/model .* does not exist|unknown model|invalid.*model|not found for model/i.test(normalized)) return "当前模型在该代理商不可用，请重新发现模型并选择可用模型。";
	if (/page not found|not found|unknown parameter|invalid type for 'images|invalid type for "images|expected an object/i.test(normalized)) return "当前代理接口或参数格式与该能力不兼容，请检查代理商是否完整支持此图像接口。";
	if (/invalid value/i.test(normalized) && /1024x1024|1024x1536|1536x1024|auto|size/i.test(normalized)) return "当前图像尺寸参数不被该模型或代理商接受，系统需要切换到兼容尺寸。";
	if (/image generation temporarily unavailable|图像生成接口暂时不可用/i.test(raw)) return "图像生成接口暂时不可用，请稍后重试或更换可用模型。";
	if (/malformed|json|schema/i.test(normalized)) return "模型返回的结构化结果不符合预期，系统正在尝试自动修复。";
...
const IMAGE_CATEGORIES = /* @__PURE__ */ new Set(["image_generation", "image_edit"]);
const CHAT_CATEGORIES = /* @__PURE__ */ new Set([
	"text",
	"structured",
	"vision"
]);
function summarizeUsage(entries, options = {}) {
	...
	const filtered = entries.filter((entry) => {
		...
		if (options.category && options.category !== "all" && entry.category !== options.category) return false;
		if (options.quotaState && options.quotaState !== "all" && entry.quotaState !== options.quotaState) return false;
		if (options.success === "success" && !entry.ok) return false;
		if (options.success === "failed" && entry.ok) return false;
		return true;
	});
	const countBy = (key) => Object.entries(filtered.reduce((accumulator, entry) => {
	...
	return {
		...
		averageDurationMs: filtered.length > 0 ? Math.round(filtered.reduce((sum, entry) => sum + entry.durationMs, 0) / filtered.length) : 0,
...
const ROUTES = {
	...
	projectUpdate: `${API_PREFIX}/project/update`,
	projectDelete: `${API_PREFIX}/project/delete`,
	upload: `${API_PREFIX}/upload`,
	assetsReorder: `${API_PREFIX}/assets/reorder`,
	assetSetMain: `${API_PREFIX}/assets/set-main`,
	assetDelete: `${API_PREFIX}/assets/delete`,
	analyze: `${API_PREFIX}/analyze`,
	analysisSave: `${API_PREFIX}/analysis/save`,
	...
	translatePage: `${API_PREFIX}/translate-page`,
	job: `${API_PREFIX}/job`,
	jobCancel: `${API_PREFIX}/job/cancel`,
	tasks: `${API_PREFIX}/tasks`,
	taskRetry: `${API_PREFIX}/tasks/retry`,
	...
	providerTest: `${API_PREFIX}/providers/test`,
	providerDiscover: `${API_PREFIX}/providers/discover`,
	usage: `${API_PREFIX}/monitor/usage`,
	usageClear: `${API_PREFIX}/monitor/usage/clear`,
	usageDelete: `${API_PREFIX}/monitor/usage/delete`,
	batchCreate: `${API_PREFIX}/batch-create`,
	xhsPlan: `${API_PREFIX}/xiaohongshu/plan`,
	xhsGenerate: `${API_PREFIX}/xiaohongshu/generate`,
	xhsEdit: `${API_PREFIX}/xiaohongshu/edit`
};
//#endregion
//#region src/host/usage-monitor.ts
/**
...
async function readUsageEntries(storeRoot, limit = 1e3) {
	let raw;
	try {
...
async function clearUsageEntries(storeRoot) {
	try {
...
async function deleteUsageEntry(storeRoot, entryId) {
...
async function readJsonBody(req) {
	...
	for await (const chunk of req) {
		...
		if (total > 67108864) throw new Error("request body too large");
		chunks.push(buffer);
	}
...
*/
function envelope(method, handler) {
	return async (req, res) => {
		if (!guard(req, res, method)) return;
		try {
			writeJson(res, 200, {
				ok: true,
				...await handler(method === "GET" ? {} : await readJsonBody(req), req)
			});
		} catch (error) {
			const message = redactSecrets(error instanceof Error ? error.message : String(error));
			writeJson(res, 200, {
				ok: false,
				code: /429|quota|额度/i.test(message) ? "MXPAGE_HTTP_429" : /401|403|key|密钥|未配置/i.test(message) ? "MXPAGE_HTTP_401" : /not found|不存在/i.test(message) ? "MXPAGE_NOT_FOUND" : /cancel|取消/i.test(message) ? "MXPAGE_CANCELLED" : "MXPAGE_ERROR",
				message
			});
		}
	};
	...
	async function projectView(projectId) {
		...
		return {
			...
			sections: detail.sections.map((section) => ({
				...
				imageUrl: section.currentImageAssetId ? imageUrl(detail.assets.find((asset) => asset.id === section.currentImageAssetId)?.filePath ?? "") : null,
				versions: section.versions.map((version) => ({
					id: version.id,
					versionNumber: version.versionNumber,
					isActive: version.isActive,
					createdAt: version.createdAt,
					imageUrl: version.imageAssetId ? imageUrl(detail.assets.find((asset) => asset.id === version.imageAssetId)?.filePath ?? "") : null
				}))
			}))
		};
	}
	return [
		{
			...
			handler: envelope("POST", async () => {
				...
				return { projects: await Promise.all(list.map(async (project) => {
					const main = (await runtime.host.repository.asset.list({ projectId: project.id })).find((asset) => asset.isMain);
					return {
						id: project.id,
						name: project.name,
		...
		},
		{
			...
			handler: envelope("POST", async (body) => {
				...
				if (files.length < 1 || files.length > 10) throw new Error("supply 1–10 product photos");
				...
				await runtime.host.repository.project.mergeModelSnapshot(project.id, { previewConfig: {
		...
		},
		{
			...
			handler: envelope("POST", async (body) => {
				const id = asString(body.id);
				if (!id) throw new Error("id is required");
				if (!await runtime.host.repository.project.get(id)) throw new Error(`project not found: ${id}`);
				await runtime.host.repository.project.delete(id);
				await runtime.assets.removeProjectDirs(id).catch((error) => {
		...
		},
		{
			...
			handler: envelope("POST", async (body) => {
				...
				if (!projectId || !Array.isArray(ordered)) throw new Error("projectId and orderedAssetIds are required");
				let index = 0;
		...
		},
		{
			...
			handler: envelope("POST", async (body) => {
				...
				if (await runtime.host.repository.asset.isReferenced(assetId)) throw new Error("素材正被分区或版本引用，请先切换分区图片后再删除。");
				if (!await runtime.assets.deleteAssetRecord(assetId)) throw new Error(`asset not found: ${assetId}`);
				return { deleted: true };
			})
		},
		{
			...
			handler: envelope("POST", async (body) => {
				...
				if (!projectId || !fileName || !base64Data) throw new Error("projectId, fileName and base64Data are required");
		...
		},
		{
			...
			handler: envelope("POST", async (body) => {
				...
				return { analysis: (await runtime.analysis.analyzeProject(projectId, asString(body.model) ?? null)).normalizedResult };
			})
		},
		{
			kind: "exact",
			...
			handler: envelope("POST", async (body) => {
				...
				if (!projectId || !analysis || typeof analysis !== "object") throw new Error("projectId and analysis are required");
		...
		},
		{
			...
			handler: envelope("POST", async (body) => {
				const projectId = asString(body.projectId);
				if (!projectId) throw new Error("projectId is required");
				const previewConfig = body.previewConfig ?? {};
				const result = await runtime.planner.planSections(projectId, {
					modelId: asString(body.model) ?? null,
					previewConfig: {
						heroImageCount: asNumber(previewConfig.heroImageCount) ?? config.defaultHeroCount,
						detailSectionCount: asNumber(previewConfig.detailSectionCount) ?? config.defaultDetailCount,
						imageAspectRatio: asString(previewConfig.imageAspectRatio) ?? config.defaultDetailAspectRatio,
						contentLanguage: asString(previewConfig.contentLanguage) ?? config.defaultLanguage
					},
		...
		},
		{
			...
			handler: envelope("POST", async (body) => {
				const projectId = asString(body.projectId);
				if (!projectId) throw new Error("projectId is required");
				return { visualStyleGuide: (await runtime.planner.regenerateVisualStyleGuide(projectId, asString(body.model) ?? null)).visualStyleGuide };
			})
		},
		{
			kind: "exact",
			...
			handler: envelope("POST", async (body) => {
				...
				return { section: await runtime.planner.updateSection(sectionId, patch) };
			})
		},
		{
			kind: "exact",
			...
			handler: envelope("POST", async (body) => {
				const projectId = asString(body.projectId);
				if (!projectId) throw new Error("projectId is required");
				return { section: await runtime.planner.createSection(projectId, {
		...
		},
		{
			...
			handler: envelope("POST", async (body) => {
				...
				if (!projectId || !Array.isArray(ordered)) throw new Error("projectId and orderedSectionIds are required");
				await runtime.planner.reorderSections(projectId, ordered);
				return {};
			})
		},
		{
			...
			handler: envelope("POST", async (body) => {
				...
				const result = await (body.regenerate === true ? runtime.generation.regenerateSectionImage : runtime.generation.generateSectionImage)(projectId, sectionId, asString(body.model) ?? null);
				return {
		...
		},
		{
			...
			handler: envelope("POST", async (body) => {
				const projectId = asString(body.projectId);
				const sectionId = asString(body.sectionId);
				const editMode = asString(body.editMode);
				if (!projectId || !sectionId || !editMode) throw new Error("projectId, sectionId and editMode are required");
				if (editMode === "translate" && !asString(body.targetLanguage)) throw new Error("targetLanguage is required for mode=translate");
		...
		},
		{
			...
			handler: envelope("POST", async (body) => {
				...
				const sectionIds = (await runtime.host.repository.section.list(projectId)).filter((section) => mode === "all" ? true : !section.currentImageAssetId).sort((a, b) => a.order - b.order).map((section) => section.id);
		...
		},
		{
			...
			handler: envelope("POST", async (body) => {
				...
				if (!projectId || !targetLanguage) throw new Error("projectId and targetLanguage are required");
				const targets = (await runtime.host.repository.section.list(projectId)).filter((section) => section.currentImageAssetId).sort((a, b) => a.order - b.order);
		...
		},
		{
			...
			handler: envelope("GET", async (_body, req) => {
				...
				if (runnerState && runnerState !== "unknown") return {
					state: runnerState,
					progress: null,
					error: null
				};
		...
		},
		{
			...
			handler: envelope("GET", async (_body, req) => {
				...
				const projects = projectId ? [{ id: projectId }] : await runtime.host.repository.project.list({
					limit: 100,
					includeSystem: true
				});
				return { tasks: (await Promise.all(projects.map((project) => runtime.host.repository.task.list(project.id, limit)))).flat().sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).slice(0, limit).map((task) => ({
					id: task.id,
		...
		},
		{
			...
			handler: envelope("POST", async (body) => {
				...
				if (task.status === "RUNNING" || task.status === "PENDING") throw new Error("任务仍在进行中，请先取消再重试。");
				const input = task.inputPayload ?? {};
				if ((task.taskType === "GENERATE" || task.taskType === "REGENERATE") && task.sectionId) {
					const result = await runtime.generation.generateSectionImage(task.projectId, task.sectionId);
					return {
						retried: true,
						taskType: task.taskType,
						versionId: result.version.id,
				...
				if (task.taskType === "TRANSLATE_PAGE") {
					...
					const targets = (await runtime.host.repository.section.list(task.projectId)).filter((section) => section.currentImageAssetId).sort((a, b) => a.order - b.order);
					if (targets.length === 0) throw new Error("没有已出图的分区可翻译");
					const total = targets.length;
		...
		},
		{
			...
			handler: envelope("GET", async (_body, req) => {
				...
				if (!sectionId) throw new Error("sectionId is required");
				return { versions: (await runtime.generation.listSectionVersions(sectionId)).map((version) => ({
					id: version.id,
					versionNumber: version.versionNumber,
					isActive: version.isActive,
					createdAt: version.createdAt,
		...
		},
		{
			...
			handler: envelope("POST", async (body) => {
				const projectId = asString(body.projectId);
				if (!projectId) throw new Error("projectId is required");
				if (asString(body.format) === "json") return {
					format: "json",
					project: await runtime.host.repository.project.getDetail(projectId)
				};
		...
		},
		{
			...
			handler: async (req, res) => {
				...
				try {
					...
					const contentType = lower.endsWith(".jpg") || lower.endsWith(".jpeg") ? "image/jpeg" : lower.endsWith(".webp") ? "image/webp" : lower.endsWith(".svg") ? "image/svg+xml" : "image/png";
					res.writeHead(200, {
		...
		},
		{
			...
			handler: envelope("POST", async () => {
				...
				return {
					...
					imageModels: resolved.models.filter((model) => model.capabilities.image_gen || model.capabilities.image_edit).map((model) => model.modelId),
					textModels: resolved.models.filter((model) => model.capabilities.text).map((model) => model.modelId),
					visionModels: resolved.models.filter((model) => model.capabilities.vision).map((model) => model.modelId),
					channels: (config.channels ?? []).map((channel) => ({
						id: channel.id,
						label: channel.label ?? channel.id,
						baseUrl: channel.baseUrl,
						disabled: channel.disabled === true,
						hasKey: Boolean((channel.apiKeyEnv ? process.env[channel.apiKeyEnv] : void 0) ?? channel.apiKey),
		...
		},
		{
			...
			handler: envelope("POST", async (body) => {
				...
				return { result: await new OpenAICompatibleAdapter(channel.baseUrl, apiKey, runtime.logger).testConnection() };
			})
		},
		{
			kind: "exact",
			...
			handler: envelope("POST", async (body) => {
				...
				const apiKey = readChannelKey(channel);
				if (!apiKey) throw new Error(`渠道 ${channel.id} 未配置密钥。`);
				const detected = normalizeDetectedModels((await new OpenAICompatibleAdapter(channel.baseUrl, apiKey, runtime.logger).listModels()).map((item) => ({
					id: item.id,
		...
		},
		{
			...
			handler: envelope("GET", async (_body, req) => {
				const params = query(req);
				return { summary: summarizeUsage((await readUsageEntries(runtime.storeRoot)).map(humanizeEntry), {
					hours: Number(params.get("hours") ?? 24) || 24,
					limit: Number(params.get("limit") ?? 50) || 50,
					page: Number(params.get("page") ?? 1) || 1,
					projectId: params.get("projectId") || void 0,
					category: params.get("category") || "all",
					quotaState: params.get("quotaState") || "all",
					success: params.get("success") || "all"
				}) };
			})
		},
		{
		...
		},
		{
			...
			handler: envelope("POST", async (body) => {
				...
				return { plan: await runtime.xiaohongshu.planXiaohongshuPost({
					topic,
					imageCount: asNumber(body.imageCount) ?? 5,
					imageAspectRatio: asString(body.aspectRatio) ?? "3:4"
				}) };
			})
		},
		{
			...
			handler: envelope("POST", async (body) => {
				...
				return { pages: await runtime.xiaohongshu.generateXiaohongshuImages(plan, void 0, { imageAspectRatio: asString(body.aspectRatio) }) };
			})
		},
		{
			kind: "exact",
			...
			handler: envelope("POST", async (body) => {
				...
				return { edited: await runtime.xiaohongshu.editXiaohongshuImage({
					imageUrl: imageUrlValue,
					prompt,
					imageAspectRatio: asString(body.aspectRatio)
				}) };
			})
		}
	];
...
function registerMxpageTools(ctx, config, runtime) {
	...
	*/
	...
	disposers.push(ctx.tools.register(defineTool({
		...
		description: "Translate the whole page in the background: every section that already has an image gets a translate edit (all in-image text re-lettered into the target language), producing new versions. Returns a job id — poll with mxpage_job_status.",
		parameters: {
			project_id: {
				type: "string",
				required: true
		...
		async execute(args, exec) {
			try {
				...
				const targets = (await host.repository.section.list(projectId)).filter((section) => section.currentImageAssetId).sort((a, b) => a.order - b.order);
				if (targets.length === 0) return fail("MXPAGE_STATE", "no generated sections to translate");
				if (exec.signal?.aborted) return fail("MXPAGE_CANCELLED", "canceled before start");
				const total = targets.length;
				return ok({
					kind: "background",
	...
	disposers.push(ctx.tools.register(defineTool({
		...
		async execute(args) {
			try {
				const project = await host.repository.project.update(args.project_id, {
					name: args.name?.trim() || void 0,
					platform: args.platform?.trim() || void 0,
					style: args.style?.trim() || void 0,
	...
	disposers.push(ctx.tools.register(defineTool({
		...
		description: "Delete a project and every workspace file it owns (uploads, generated images, exports). This is destructive and irreversible — confirm with the user first.",
		parameters: {
			project_id: {
				type: "string",
				required: true
		...
		async execute(args) {
			try {
				if (args.confirm !== true) return fail("MXPAGE_CONFIRM", "pass confirm: true to delete the project");
				const projectId = args.project_id;
				if (!await host.repository.project.get(projectId)) return fail("MXPAGE_NOT_FOUND", `unknown project ${projectId}`);
	...
	disposers.push(ctx.tools.register(defineTool({
		...
		description: "Promote one of a project's uploaded product photos to the main image (the hero reference for every generation).",
		parameters: {
			project_id: {
				type: "string",
				required: true
			...
			asset_id: {
				type: "string",
				description: "Asset id from the panel or project status."
			},
			image_path: {
				type: "string",
				description: "Workspace path of the asset (alternative to asset_id)."
			}
		},
		output: {
			schema: objectSchema,
		...
		async execute(args) {
			try {
				const projectId = args.project_id;
				let assetId = args.asset_id?.trim();
				if (!assetId) {
					const imagePath = args.image_path?.trim();
					if (!imagePath) return fail("MXPAGE_ARGS", "asset_id or image_path is required");
					const match = (await host.repository.asset.list({ projectId })).find((asset) => asset.filePath === imagePath || asset.fileName === imagePath);
	...
	disposers.push(ctx.tools.register(defineTool({
		...
		description: "Summarize recent MxPage API usage from the workspace ledger: totals, success rate, chat vs image calls, rate/spending-limit hits, top models. Read-only diagnostic for quota debugging.",
		parameters: { hours: {
			type: "number",
		...
		async execute(args) {
			try {
				const hours = clamp(args.hours, 1, 720, 24);
				const summary = summarizeUsage(await readUsageEntries(storeRoot), {
					hours,
					limit: 20
				});
				return ok({
					...
					recentErrors: summary.recentEntries.filter((entry) => !entry.ok).slice(0, 5).map((entry) => ({
						at: entry.at,
						model: entry.model,
						status: entry.status,
...
function apply(ctx, config) {
	...
	let runtime = createMxpageRuntime(current, {
		jobs,
		resolveOwner: () => {
			try {
				return ctx.get("agent") ?? void 0;
			} catch {
				return;
			}
		}
	...
	*/
	...
	function reRegisterTools() {
		...
		ctx.inject(["tools", "attachments"], (tctx) => {
			tctx.effect(() => {
				const disposers = registerMxpageTools(tctx, resolveConfig(), runtime);
	...
	ctx.inject(["webServer"], (wctx) => {
		wctx.effect(() => {
			...
			const disposers = makeMxpageRoutes({
				runtime: new Proxy({}, { get: (_target, prop) => runtime[prop] }),
				config: new Proxy({}, { get: (_target, prop) => resolveConfig()[prop] }),
