...
function createMxpageRuntime(config, options = {}) {
	...
	const runner = options.jobs ? createJobsTaskRunner({
		jobs: options.jobs,
		repository,
	...
	return [
		{
			...
			handler: envelope(async () => {
				...
				return { projects: await Promise.all(list.map(async (project) => {
					const main = (await host.repository.asset.list({ projectId: project.id })).find((asset) => asset.isMain);
					return {
						id: project.id,
						name: project.name,
		...
		},
		{
			...
			handler: envelope(async (body) => {
				...
				return { analysis: (await analysis.analyzeProject(projectId, asString(body.model) ?? null)).normalizedResult };
			})
		},
		{
			kind: "exact",
		...
		},
		{
			...
			handler: envelope(async (body) => {
				const projectId = asString(body.projectId);
				if (!projectId) throw new Error("projectId is required");
				return { visualStyleGuide: (await planner.regenerateVisualStyleGuide(projectId, asString(body.model) ?? null)).visualStyleGuide };
			})
		},
		{
			kind: "exact",
			...
			handler: envelope(async (body) => {
				...
				return { section: await planner.updateSection(sectionId, patch) };
			})
		},
		{
			kind: "exact",
			...
			handler: envelope(async (body) => {
				const projectId = asString(body.projectId);
				if (!projectId) throw new Error("projectId is required");
				return { section: await planner.createSection(projectId, {
		...
		},
		{
			...
			handler: envelope(async (body) => {
				...
				await planner.reorderSections(projectId, ordered);
				return {};
			})
		},
		{
			...
			handler: envelope(async (body) => {
				...
				const result = await (body.regenerate === true ? generation.regenerateSectionImage : generation.generateSectionImage)(projectId, sectionId, asString(body.model) ?? null);
				return {
		...
		},
		{
			...
			handler: envelope(async (body) => {
				...
				const sectionIds = (await host.repository.section.list(projectId)).filter((section) => mode === "all" ? true : !section.currentImageAssetId).sort((a, b) => a.order - b.order).map((section) => section.id);
		...
		},
		{
			...
			handler: envelope(async (_body, req) => {
				...
				if (!sectionId) throw new Error("sectionId is required");
				return { versions: (await generation.listSectionVersions(sectionId)).map((version) => ({
					id: version.id,
					versionNumber: version.versionNumber,
					isActive: version.isActive,
					createdAt: version.createdAt,
		...
		},
		{
			...
			handler: envelope(async (body) => {
				...
				if (asString(body.format) === "json") return {
					format: "json",
					project: await host.repository.project.getDetail(projectId)
				};
		...
		},
		{
			...
			handler: envelope(async (body) => {
				...
				return { plan: await xiaohongshu.planXiaohongshuPost({
					topic,
		...
		},
		{
			...
			handler: envelope(async (body) => {
				...
				return { pages: await xiaohongshu.generateXiaohongshuImages(plan, void 0, { imageAspectRatio: asString(body.aspectRatio) }) };
			})
		},
		{
			kind: "exact",
			...
			handler: envelope(async (body) => {
				...
				return { edited: await xiaohongshu.editXiaohongshuImage({
...
function apply(ctx, config) {
	...
	if (webServer?.register) ctx.effect(() => {
